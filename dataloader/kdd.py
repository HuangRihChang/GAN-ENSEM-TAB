...
class KDD_dataset(Dataset):
    def __init__(self, opt, mode):
        ...
        print('processing KDD99...')
        if mode=='train':
            data, label = get_train(opt)
        else:
            data, label = get_test(opt)
        self.data = data
        self.label = label
        print('finish KDD99...')
    ...
    def __getitem__(self, index):
       return np.float32(self.data[index]), np.float32(self.label[index])

def get_train(opt):
    """Get training dataset for KDD 10 percent"""
    return _get_adapted_dataset(opt, "train")

def get_test(opt):
    """Get testing dataset for KDD 10 percent"""
    return _get_adapted_dataset(opt,"test")
...
def _get_dataset(opt):
    ...
    df = pd.read_csv(os.path.join(opt.dataroot, 'KDD99', "kddcup.data_10_percent_corrected"), header=None, names=col_names)
...
def _get_adapted_dataset(opt, split):
...
def _to_xy(df, target):
    ...
    for x in df.columns:
        if x != target:
            result.append(x)
    dummies = df[target]
    return df.values.astype(np.float32), dummies.values.astype(np.float32)
